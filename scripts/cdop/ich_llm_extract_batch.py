"""
ICH LLM Extraction - Batch processing for Tier A+B documents
"""
import sys
import os
import json
...
OUTPUT_FILE = '/Users/karlg/Documents/Repos/_cedop/output/cdop/ich_llm_extractions_tier_ab.json'
...
def main():
    ...
    # Get Tier A and B document IDs
    tier_a_ids = triage["tiers"]["A"]["docs"]
    tier_b_ids = triage["tiers"]["B"]["docs"]
    target_ids = tier_a_ids + tier_b_ids

    print(f"Processing {len(target_ids)} documents (Tier A: {len(tier_a_ids)}, Tier B: {len(tier_b_ids)})")
    ...
    for i, ich_id in enumerate(target_ids):
