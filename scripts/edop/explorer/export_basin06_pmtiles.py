"""Export basin06 geometry to GeoJSON, then invoke tippecanoe to build basin06.pmtiles.

Output: app/static/explorer/basin06.pmtiles
Run from the repo root: python scripts/edop/explorer/export_basin06_pmtiles.py
"""
...
# Add repo root to path so db_connect() resolves
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..'))
from app.db.connection import db_connect

TIPPECANOE = '/opt/homebrew/bin/tippecanoe'
OUT_PMTILES = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'app', 'static', 'explorer', 'basin06.pmtiles')


def export_geojson(conn, path):
    print("Querying basin06 geometry…", flush=True)
...
def main():
    ...
    try:
        ...
        if os.path.exists(geojson_path):
            ...
            print(f"  Cleaned up temp GeoJSON.", flush=True)

    print("Done. Add basin06.pmtiles to your rsync list for server deploy.")
