...
EDOPS signatures represent computable and configurable summarizations of environmental conditions at given locations utilizing rigorously developed data. EDOP is a research instrument for environmental characterization, not a predictive model of settlement viability. The primary question is whether signatures capture genuine environmental structure — whether places that humans have actually settled cluster together in signature space more than chance would predict. The methodology is correspondence testing, not prediction: we are not asking "which locations does the model say are favorable?" but "do known settlement hearths share environmental structure that the signatures detect?"

Correspondence is an existence proof of the instrument's environmental sensitivity, not an optimization target. Residuals are findings: a place with dense historical settlement but an apparently sparse signature points to a dimension the current model doesn't capture — maritime affordances, mobile subsistence strategies, or strategic imperatives like trade and defense. A place with a rich signature but thin settlement record raises different questions about connectivity, competitive exclusion, or historical contingency. Anomalies reveal as much as matches.

The primary correspondence experiment uses <a href="https://d-place.org" target="_blank">*D-PLACE*</a>: a global Mantel test asking whether environmental similarity in signature space correlates with geographic proximity across the roughly 1,400 society locations — an existence proof of the instrument's sensitivity before any settlement-specific testing. Secondary datasets include (ii) polygon geometry for over 800 temporally scoped historical polities from the <a href="https://github.com/Seshat-Global-History-Databank/cliopatria" target="_blank">Cliopatria dataset</a> developed by the <a href="https://seshatdatabank.info/" target="_blank">Seshat *Global History Databank* project</a>, and (iii) temporally scoped point locations for roughly 1,700 historical settlements spanning 6,000 years, developed by Reba, et al (2016).

One important caveat is that the current EDOP signature is built from terrestrial data and will show the strongest correspondence for fixed settlements relying on terrestrial resources. Structural blind spots follow from this directly. Signatures across the Eurasian steppe will characterize the terrain accurately — and yet the steppe supported intensive occupation by mobile cultures whose movement was adaptive to, not despite, those conditions. The terrestrial signature for Tierra del Fuego will correctly characterize harsh terrestrial conditions — yet the rich marine resources allowed the Yaghan to thrive there for millennia, and the presence of guanaco in the interior supported the Selknam societies. As described in Section 4, these are not anomalies to be explained away: the absence of correspondence points directly at a missing dimension, and identifying it — coastality, mobility, strategic access — is itself a finding.
...
These core datasets, introduced above in the context of EDOP correspondence testing, will see further use in seeking patterns of co-relation:
...
Refining the EDOP component of Computing Place is now the top priority--work that includes three efforts mentioned earlier: studies of scale sensitivity and upstream/downstream distance weighting, and correspondence testing against historical settlement hearths and ethnographic society datasets.
